...
export async function extractNewsContent(
  ...
  if (
    post.sourceUrl && !post.domain.includes("reddit.com") &&
    ...
    try {
      ...
      if (
        ...
        // }
        post.description =
          `(Content from external link: ${post.sourceUrl} - needs manual summary or improved extractor)`;
      } else {
        post.description =
          `Failed to fetch or not HTML content from ${post.sourceUrl}. Status: ${response.status}`;
      }
    } catch (error) {
      console.error(
        `Error fetching external content from ${post.sourceUrl}:`,
        error,
      );
      post.description =
        `Error fetching content from link. Visit: ${post.sourceUrl}`;
    }
  } else if (!post.textContent) {
    post.description =
      "No text content provided in Reddit post or linked article.";
  }
