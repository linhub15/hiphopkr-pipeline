...
function generateMarkdownContent(post: ProcessedRedditPost): string {
  ...
  if (post.artist) frontmatter += `artist: "${post.artist.replace(/"/g, '\\\\"')}"\n`;
  if (post.trackOrAlbumTitle) frontmatter += `trackOrAlbumTitle: "${post.trackOrAlbumTitle.replace(/"/g, '\\\\"')}"\n`;
  if (post.featureType) frontmatter += `featureType: ${post.featureType}\n`;
  if (post.releaseDate) frontmatter += `releaseDate: ${post.releaseDate}\n`;
  if (post.producers && post.producers.length > 0) frontmatter += `producers: [${post.producers.map(p => `"${p.replace(/"/g, '\\\\"')}"`).join(", ")}]\n`;
  if (post.albumCoverUrl) frontmatter += `albumCoverUrl: ${post.albumCoverUrl}\n`;
  if (post.spotifyLink) frontmatter += `spotifyLink: ${post.spotifyLink}\n`;
  if (post.appleMusicLink) frontmatter += `appleMusicLink: ${post.appleMusicLink}\n`;
  frontmatter += "---\n\n";
...
export async function writeDebugMarkdownFile(post: ProcessedRedditPost) {
  ...
  // Sanitize filename
  const filename = `${post.id}_${post.title.replace(/[^a-z0-9_\-]+/gi, '_').toLowerCase()}.md`;
