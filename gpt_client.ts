...
export async function generateSynopsis(post: ProcessedRedditPost): Promise<ProcessedRedditPost> {
  if (!config.openai.apiKey) {
    console.warn("OpenAI API key not configured. Skipping synopsis generation.");
    return post;
  }
  ...
  if (post.featureType === 'news' || post.featureType === 'rumor') {
    ...
    promptContent = `Generate a concise, SEO-friendly news summary (around 100-150 words) for a blog post about the following Korean hip-hop news. Highlight key information. News content: "${post.description}"`;
  } else if (['track', 'album', 'ep', 'mv'].includes(post.featureType!)) {
    if (!post.artist || !post.trackOrAlbumTitle) return post; // Not enough info
    promptContent = `Generate a short, engaging, and SEO-friendly synopsis (around 100-150 words) for a blog post about the Korean hip-hop ${post.featureType} release titled "${post.trackOrAlbumTitle}" by "${post.artist}". Mention the artist and title. If available, incorporate release date: ${post.releaseDate}. Available description/details: "${post.description || 'None'}"`;
  } else {
  ...
  try {
    const response = await fetch(config.openai.apiUrl, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "You are a helpful assistant that writes concise summaries for a Korean Hip-Hop news and music blog." },
