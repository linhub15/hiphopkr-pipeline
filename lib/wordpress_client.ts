...
async function uploadMediaToWordPress(imageUrl: string, title: string, authToken: string): Promise<number | null> {
    ...
    try {
        ...
        const originalFileName = imageUrl.substring(imageUrl.lastIndexOf('/') + 1) || `${title.replace(/\s+/g, '-')}-cover`;
        // Ensure a common extension if missing, or derive from blob.type
        const fileName = originalFileName.includes('.') ? originalFileName : `${originalFileName}.${imageBlob.type.split('/')[1] || 'jpg'}`;
        ...
        const mediaResponse = await fetch(`${config.wordpress.endpoint}/wp/v2/media`, {
            method: "POST",
            headers: {
        ...
        if (!mediaResponse.ok) {
            ...
            console.error(`Failed to upload media to WordPress: ${mediaResponse.status} - ${errorText}`);
            return null;
        }
...
function generatePostContent(post: ProcessedRedditPost): string {
  ...
  if (post.albumCoverUrl && post.featureType !== 'news' && post.featureType !== 'rumor') {
  ...
  if (post.artist && post.trackOrAlbumTitle && post.featureType !== 'news' && post.featureType !== 'rumor') {
      content += `<p><strong>Artist:</strong> ${post.artist}</p>\n`;
      content += `<p><strong>Title:</strong> ${post.trackOrAlbumTitle}</p>\n`;
  }
  if (post.releaseDate && post.featureType !== 'news' && post.featureType !== 'rumor') {
      content += `<p><strong>Release Date:</strong> ${post.releaseDate}</p>\n`;
  }
  if (post.producers && post.producers.length > 0 && post.featureType !== 'news' && post.featureType !== 'rumor') {
      content += `<p><strong>Producer(s):</strong> ${post.producers.join(", ")}</p>\n\n`;
  }
  ...
  if (post.spotifyLink || post.appleMusicLink) {
    ...
    if (post.spotifyLink) {
      content += `  <li><a href="${post.spotifyLink}" target="_blank" rel="noopener noreferrer">Spotify</a></li>\n`;
    }
    if (post.appleMusicLink) { // Assuming you might add this later
      content += `  <li><a href="${post.appleMusicLink}" target="_blank" rel="noopener noreferrer">Apple Music</a></li>\n`;
    }
    content += `</ul>\n\n`;
  }

  content += `<p><em>Source: <a href="${post.redditLink}" target="_blank" rel="noopener noreferrer">r/khiphop on Reddit</a>`;
  if (post.sourceUrl !== post.redditLink && !post.sourceUrl.includes('i.redd.it') && !post.sourceUrl.includes('v.redd.it')) {
      content += ` | <a href="${post.sourceUrl}" target="_blank" rel="noopener noreferrer">Original Source</a>`;
  }
  content += `</em></p>\n`;
...
function generatePostTitle(post: ProcessedRedditPost): string {
    ...
    else if (post.featureType === 'album' || post.featureType === 'ep') prefix = "New Album/EP:";
...
async function getTermIds(name: string, taxonomy: 'category' | 'post_tag', authToken: string): Promise<number[]> {
    ...
    if (lowerFeatureType && termMap[lowerFeatureType] && termMap[lowerFeatureType][taxonomy]) {
...
export async function createWordPressDraft(post: ProcessedRedditPost) {
  const authToken = btoa(`${config.wordpress.username}:${config.wordpress.password}`);
  let featuredMediaId: number | null = null;

  // Upload album cover as featured image if it's a music release
  if (post.albumCoverUrl && ['track', 'album', 'ep', 'mv'].includes(post.featureType!)) {
      console.log(`Uploading media for: ${post.title}`);
      featuredMediaId = await uploadMediaToWordPress(post.albumCoverUrl, `${post.artist || ''} ${post.trackOrAlbumTitle || post.title}`, authToken);
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to create WordPress draft: ${response.status} - ${JSON.stringify(errorData)}`);
    }
    const createdPost = await response.json();
    console.log(`WordPress draft created: ${createdPost.id} - ${createdPost.link}`);
    return createdPost;
  } catch (error) {
